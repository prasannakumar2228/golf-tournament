...
public class ScoreService {
    ...
    public Score submitScore(String playerName, int holeNumber, int strokes) {
        ...
        List<Score> scoresForPlayer = getScoresForPlayer(playerName);
        if(scoresForPlayer.stream().anyMatch(score -> score.getHoleNumber() == holeNumber)) {
        	throw new InvalidHoleException("Player " + playerName + " has already completed hole number " + holeNumber);
        }
